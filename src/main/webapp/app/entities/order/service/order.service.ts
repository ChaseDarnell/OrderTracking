...
export class OrderService {
  ...
  protected convertDateFromClient(order: IOrder): IOrder {
    return Object.assign({}, order, {
      pickUpDate: order.pickUpDate?.isValid() ? order.pickUpDate.toJSON() : undefined,
      repairDate: order.repairDate?.isValid() ? order.repairDate.toJSON() : undefined,
      deliveryDate: order.deliveryDate?.isValid() ? order.deliveryDate.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.pickUpDate = res.body.pickUpDate ? dayjs(res.body.pickUpDate) : undefined;
      res.body.repairDate = res.body.repairDate ? dayjs(res.body.repairDate) : undefined;
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((order: IOrder) => {
        order.pickUpDate = order.pickUpDate ? dayjs(order.pickUpDate) : undefined;
        order.repairDate = order.repairDate ? dayjs(order.repairDate) : undefined;
        order.deliveryDate = order.deliveryDate ? dayjs(order.deliveryDate) : undefined;
      });
    }
    return res;
  }
